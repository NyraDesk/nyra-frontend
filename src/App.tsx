...
Per ogni email da inviare, genera SOLO questo JSON:
{
  ...
  "body": "CONTENUTO COMPLETO E DETTAGLIATO dell'email - testo professionale con dettagli del progetto, servizi offerti, prezzi, tempistiche, proposte concrete"
}
          ...
          if (emailMatches && emailMatches.length > 0) {
            ...
            for (let index = 0; index < emailMatches.length; index++) {
              ...
              try {
                ...
                console.log(`📧 Processando email ${index + 1}`);
...
Il Team NYRA`;
        ...
        if (aiResponse.includes('"action": "send-email"') || aiResponse.toLowerCase().includes('json')) {
          ...
          if (emailMatches && emailMatches.length > 0) {
            ...
            for (let index = 0; index < emailMatches.length; index++) {
              ...
              try {
                const emailAction = JSON.parse(jsonStr);
                console.log(`📧 Email ${index + 1}:`, emailAction);
                
                // Usa il contenuto completo generato dall'AI
                const templateBody = emailAction.body;

                const emailData = {
                  email: emailAction.to,
                  subject: emailAction.subject,
                  body: templateBody,
                  suggestedSubject: emailAction.subject,
                  suggestedBody: templateBody,
                  emailType: 'draft',
                  to: emailAction.to,
                  cc: emailAction.cc || '',
            ...
            // Mostra tutte le bozze email create
            if (allEmails.length > 0) {
              setEmailPreviewData({
                categorizedEmails: allEmails,
                summary: `${allEmails.length} email personalizzate`
              });
              setShowEmailPreview(true);
        ...
        type: 'normal'
          ...
          return; // NON MOSTRARE LA RISPOSTA ORIGINALE
        }
...
SE L'UTENTE CHIEDE EMAIL: Genera immediatamente i JSON send-email per tutti i clienti con "Mail Inviata: No".`;
      ...
      try {
        ...
        if (aiResponse.includes('"action": "send-email"')) {
          ...
          if (emailMatches && emailMatches.length > 0) {
            console.log('📧 Email trovate nella risposta normale:', emailMatches.length);
            
            // ACCUMULA TUTTE LE EMAIL PRIMA DI MOSTRARE IL PREVIEW
            const allEmails = [];
            ...
            for (let index = 0; index < emailMatches.length; index++) {
              ...
              try {
                const emailAction = JSON.parse(jsonStr);
                console.log(`📧 Processando email normale ${index + 1}`);
...
Il Team NYRA`;
                ...
                // Aggiungi alla lista delle email (non mostrare ancora)
                allEmails.push(emailData);
